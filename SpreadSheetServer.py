...
class SpreadSheetServer:
    ...
    def handle_request(self, request, socket):
        try:
            ...
            if method == "insert":
                ...
                affected_id = request.get("affected_id")
                index = request.get("index")
                if affected_id and index is not None:
                    # Recalculate the finger table entry
                    finger_id = (self.node_id + 2**index) % MAX_KEY
                    if finger_id == affected_id:
                        successor_node = self.find_successor(finger_id)
                        self.finger_table[index][1] = successor_node.host
                        self.finger_table[index][2] = successor_node.port
                        self.finger_table[index][3] = successor_node.socket
                        print(f"Updated finger table index {index} for affected ID {affected_id}")
    ...
    def _notify_affected_nodes(self):
        """ Notify all affected nodes to update their finger tables. """
        for i in range(FINGER_NUM):
            target_id = (self.node_id - 2**i + MAX_KEY) % MAX_KEY  # Calculate the target ID for affected nodes
            responsible_node = self.find_predecessor(target_id)  # Find the responsible node for this target ID

            if responsible_node:
                try:
                    # Notify the responsible node to update its finger table
                    request = {"method": "update_finger_table", "affected_id": self.node_id, "index": i}
                    self.send_message(responsible_node.socket, request)
                    print(f"Notified node {responsible_node.node_id} to update finger table for index {i}")
                except Exception as e:
                    print(f"Failed to notify node {responsible_node.node_id} for index {i}: {e}")
